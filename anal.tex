...
rate with correlated noise included.

The cells are much less than \SI1{mm} wide, so the DiCT pulse starts within
\SI1{ps} of the originating pulse, while as we saw in \autoref{ch:snr} even
just the peak of the pulse lasts $\sim\SI{10}{ns}$. This means that the effect
of the DiCT is multiplying the height of the pulse by an integer factor,
...
\marginpar{Mention eCT saying we do not treat it.}
...
9.5 &     AP &   1059 &  No &     & Geom. &                 & 0.435 \pm 0.034 &                 & 43.5 \pm 3.4 &   0.77 \pm 0.11 &  45 & 7 & {<1e-6} \\
    ...
    p-value less than 1e6, the errors on the fit parameters and on the derived
    quantities are rescaled with $\sqrt{\chi^2/\mathrm{dof}}$. `OF' stands for
    overflow bin included, while `FZ' stands for 0~PE bin ``fixed''. ``Prob.''
    is the probability of having more than 1~PE under the model, while ``pe''
    is the average number of PE minus~1.}
...
\marginpar{Explain that the correlated noise is important for saturation, and
that assuming degradation it can diverge if too high initially, cite gola to
say that 0.5 may be the maximum acceptable, and cite the Yellow Book to say the
maximum accepted by the specs. See logbook May 17.}
...
\marginpar{Make a reference to \autoref{ch:darkside}. And I must look at the
VETO readout, I may be saying something imprecise.}
