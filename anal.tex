...
logarithmic range of filter lengths: \SI{16}{ns}, \SI{32}{ns}, \ldots,
...
\marginpar{Currently the range is from \SI{64}{ns} to \SI{2048}{ns} in the
cached analysis and so in the plots, but the one stated in the text in the
code that would be run if the cache was deleted. Also, when I fix the
template stuff, the chapter for the truncation will be \autoref{ch:snr}.}
...
the diode lowers the potential barrier than a carrier must overcome to become
...
% Spiegare per bene come faccio i fit bayesiani con minimi quadrati e cosa
% significa statisticamente correggere con sqrt(chi2/dof)

% devo usare mainnpebackup==1 anziché mainnpe e vedere se cambia qualcosa,
% magari questa variabile la aggiungo a quelle predefinite. Però come faccio a
% fare minorpos-mainpos? Potrei riportare il mainpos da un filtro all'altro
% usando gli offset. posY = posX - offsetX + offsetY.

% aggiungere il fit con due tau. plottarli insieme. devo inventare un metodo
% generico per fare due fit insieme. Idea: quando pmf_or_cdf e prior sono liste,
% fa più fit. gli altri parametri dovrebbero riguardare solo i dati quindi non
% li vettorizzo. Per fare il modello con più tau: la cdf vede se tau è un
% array, se è un array estrae anche un array con le normalizzazioni e calcola
% il modello multicomponente.
